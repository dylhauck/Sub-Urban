...
/* ---------- PRODUCT GALLERY LOGIC ----------
   Change main image + thumbnails when color/size changes.

   images[color][size] => array of 6 URLs (first one becomes main).
   If a size-specific array is missing, it falls back to images[color].default.
*/
const images = {
  'soft-pink': {
    default: [
...
let currentSize  = 'XS'; // used if you later add size-specific images
...
// Hook up sizes (for future size-specific imagery)
document.getElementById('sizes')?.addEventListener('change', (e) => {
  if(e.target.name === 'size') {
    currentSize = e.target.value;
    updateGallery(false); // don’t scroll jump, just swap
  }
});
...
// Build thumbs & set main from current color/size
function updateGallery(scrollToTop=true){
  const list = images[currentColor];
  const set  = (list[currentSize] && list[currentSize].length) ? list[currentSize] : list.default;
  const thumbs = document.getElementById('thumbs');
  thumbs.innerHTML = set.slice(0,6).map(url => (
